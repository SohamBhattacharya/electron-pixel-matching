...
namespace Common
{
    ...
    int gerInTimePileupSummaryInfoIdx(edm::Handle <std::vector <PileupSummaryInfo> > hv_pileUp)
    {
        // Start from the end to reach the In-time bunch-crossing quicker
        for(int iPileUp = (int) hv_pileUp->size() - 1; iPileUp >= 0; iPileUp--)
        {
            auto const& pileUpInfo = (*hv_pileUp)[iPileUp];
    ...
    std::vector <double> getMinDeltaR(
        ...
        double defaultVal = Constants::DEFAULT_LARGE_DR
    )
    {
    ...
    void getHardestInCone(
        std::vector<T1> &v_obj1,
        std::vector<T2> &v_obj2,
        std::vector <int> &v_obj1_matchedObj2_index, // Will be of v_obj1.size(). Will be filled with the index of the matched obj1; if not matched, then -1.
        std::vector <double> &v_obj1_matchedObj2_deltaR, // Will be of v_obj1.size(). Will be filled with dR(obj1, obj2) if matched; if not matched, then -1.
        double maxDeltaR,
        double defaultVal = Constants::DEFAULT_LARGE_DR
    )
    {
        int nObj1 = v_obj1.size();
        int nObj2 = v_obj2.size();
        ...
        {
            int obj1_idx = -1;
            double minDeltaR = Constants::DEFAULT_LARGE_DR;
            ...
            for(int iObj1 = 0; iObj1 < nObj1; iObj1++)
            {
                ...
                double deltaR = reco::deltaR(v_obj1.at(iObj1), v_obj2.at(iObj2));
                
                if(deltaR < maxDeltaR && (obj1_idx < 0 || v_obj1.at(iObj1).pt() > v_obj1.at(obj1_idx).pt()))
                {
                    obj1_idx = iObj1;
                    minDeltaR = deltaR;
                }
